...
const convertCoordinatenToCoords = (Coordinaten: string): number[] | undefined => {
  if (!Coordinaten) return undefined;

  const coords = Coordinaten.split(",").map((coord: any) => Number(coord)); // I.e.: 52.508011,5.473280;
  if (coords.length !== 2 || undefined === coords[0] || undefined === coords[1]) return undefined;

  return [coords[1], coords[0]];
}

const openRoute = (Coordinaten: string | null): void => {
  if (!Coordinaten) return;
  // Get coords from parking variable
  const coords = convertCoordinatenToCoords(Coordinaten);
  if (undefined === coords) return;
