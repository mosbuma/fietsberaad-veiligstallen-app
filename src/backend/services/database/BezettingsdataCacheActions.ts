...
export const updateBezettingCache = async (params: CacheParams) => {
    ...
    const sql = `
      INSERT INTO Bezetting_aggregated (
        timestamp_Year, timestamp_Quarter, timestamp_Month, timestamp_Week, timestamp_Day, timestamp_Hour, timestamp_Weekday,
        bikeparkID, sectionID, source, totalCheckins, totalCheckouts, totalOccupation, totalCapacity, perc_occupation, fillup, open
      )
      SELECT
        YEAR(timestamp) AS timestamp_Year,
        QUARTER(timestamp) AS timestamp_Quarter,
        MONTH(timestamp) AS timestamp_Month,
        WEEK(timestamp) AS timestamp_Week,
        DAY(timestamp) AS timestamp_Day,
        HOUR(timestamp) AS timestamp_Hour,
        WEEKDAY(timestamp) AS timestamp_Weekday,
        bikeparkID,
        sectionID,
        source,
