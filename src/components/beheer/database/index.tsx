...
import { TransactionCacheParams, TransactionCacheStatus, TransactionCacheActions, TransactionCacheResult } from "~/backend/services/database-service";
...
const DatabaseComponent: React.FC<DatabaseComponentProps> = ({ firstDate, lastDate, bikeparks }) => {
  ...
  const [cacheStatus, setCacheStatus] = useState<TransactionCacheStatus | undefined>(undefined);
  const [updateCounter, setUpdateCounter] = useState<number>(0);

  useEffect(() => {
    const fetchCacheStatus = async () => {
      const databaseParams: TransactionCacheParams = {
        action: 'status',
        allDates,
        allBikeparks,
        startDate,
      ...
      setLoading(true);
      try {
        const response = await fetch('/api/database/transactionscache', {
            method: 'POST',
            headers: {
  ...
  const handleProcessCache = (action: TransactionCacheActions) => {
    const processCache = async () => {
      ...
      try {
        const response = await fetch('/api/database/transactionscache', {
          ...
          let result = await response.json() as TransactionCacheResult;
  ...
  return (
    <div className="p-6">
      ...
      <div className="bg-gray-200 border-2  border-gray-400 p-2 pl-4 rounded">
        <h2 className="text-xl font-semibold">Transactie cache tabel</h2>
        <div className="mt-4">
